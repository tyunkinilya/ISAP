...
__about__ = 'Stemmer, based on Yandex\'s product \"Mystem\".'
...
def stem_text(text):
    ...
    text = re.sub(r'\b' + r'\b|\b'.join(stop_words) + r'\b', ' ', text)
...
def stem_texts(texts):
    ...
    for i in range(len(texts)):
        ...
        text = re.sub(r'\b' + r'\b|\b'.join(stop_words) + r'\b', ' ', text)
        text = re.sub(r'\b\w\b', ' ', text)
        text_for_mystem += text + '---'

    stemmer = Mystem()
    text_for_mystem = ' '.join(stemmer.lemmatize(text_for_mystem[:-3]))

    texts_new = text_for_mystem.split('---')
    for i in range(len(texts_new)):
        uniq = []
        for word in texts_new[i].split():
            if not word in uniq:
                uniq.append(word)
        texts_new[i] = ' '.join(uniq)
